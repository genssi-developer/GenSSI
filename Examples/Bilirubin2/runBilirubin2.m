% runBilirubin2 runs the structural identifiability analysis for the 
% 2nd version of the Bilirubin model described by
% 
%    Meshkat et al. (2014). On finding and using identifiable parameter
%    combinations in nonlinear dynamic Systems Biology models and
%    COMBOS: a novel Web implementation, PLoS ONE, 9, e110261

% Confirm execution
genssiAskForConfirmation(20000);

genssiMain('Bilirubin2',10);