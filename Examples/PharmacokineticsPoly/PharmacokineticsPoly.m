function model = PharmacokineticsPoly()
	...
	model.sym.y = [x1,x2,x3,x4,xi1];
	model.sym.xdot = [- a1*(x1 - x2) - ka*vm*x1*xi1,a2*(x1 - x2),- b1*(x3 - x4) - kc*vm*x3*xi1,b2*(x3 - x4),ka*xi1^2*(a1*(x1 - x2) + ka*vm*x1*xi1) + kc*xi1^2*(b1*(x3 - x4) + kc*vm*x3*xi1)];
end